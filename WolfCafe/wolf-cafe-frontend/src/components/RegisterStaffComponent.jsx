...
const RegisterStaffComponent = () => {
    ...
    function handleStaffRegistrationForm(e) {
		e.preventDefault();
		setSuccessMessage("");
		// Call validate form and create staff account
		if (validateForm()) {
	        const staffRegister = { username, email, password };
			registerStaffAPICall(staffRegister)
				.then((response) => {
	            	console.log(response.data);
					setSuccessMessage("The staff was registered successfully!");
					setErrors({
								username: "",
						        email: "",
						        password: "",
			...
			// Catch the various error when creating the staff
			.catch(error => {
				const errorsCopy = { ...errors };
                if (error.response.status === 403) {
                    errorsCopy.general = "Only admins and managers can create user with staff role.";
				}
  ...
  // Create the form or user interface for registering a staff account
  return (
                    ...
                    <div className='card-body'>
                        ...
                        <form>               
                            ...
                            <div className='form-group mb-3'>
                                <button className='btn btn-primary' onClick={(e) => handleStaffRegistrationForm(e)}>Submit</button>
                            </div>
                        </form>
                    </div>
                </div>
